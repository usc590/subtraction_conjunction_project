#!/usr/bin/env python3
"""
plot_conjunction_validation_results_v2.py

Paper-ready conjunction validation plots and tables.

Inputs
------
1) validation_region_summary.csv
   Example:
   ./tfce_results/conj3d_targetpairs_validation_eval_validation_region_summary.csv

2) validation_point_lookup.csv
   Example:
   ./tfce_results/conj3d_targetpairs_validation_eval_validation_point_lookup.csv

3) validation_point_seed_lookup.csv
   Example:
   ./tfce_results/conj3d_targetpairs_validation_eval_validation_point_seed_lookup.csv
...
For conjunction:
    correct seed = validation_seed_conjunction > seed_nonzero_eps

So each rerun candidate condition gets:
    n_correct / n_total

This is the number you want for paper tables:
...
Plots:
  paper_region_validation_mean_conjunction.png
  paper_region_mean_seed_nonzero_rate.png
  paper_candidate_seed_success_count.png
  paper_candidate_success_rate.png
  paper_discovery_vs_validation_conjunction.png
  paper_validation_conjunction_by_region.png

Example
-------
python plot_conjunction_validation_results_v2.py \
  --region_summary_csv ./tfce_results/conj3d_targetpairs_validation_eval_validation_region_summary.csv \
  --point_lookup_csv ./tfce_results/conj3d_targetpairs_validation_eval_validation_point_lookup.csv \
  --point_seed_lookup_csv ./tfce_results/conj3d_targetpairs_validation_eval_validation_point_seed_lookup.csv \
  --outdir ./tfce_results/conj3d_targetpairs_validation_plots_v2
"""
...
def pretty_pair_label(pair_name: str) -> str:
    if "_and_" in str(pair_name):
        a, b = str(pair_name).split("_and_", 1)
        return f"{a} ∧ {b}"
    return str(pair_name)


def short_point_label(row) -> str:
    pair = pretty_pair_label(row["pair_name"])
...
def region_label(row) -> str:
    return f"{pretty_pair_label(row['pair_name'])}\nR{int(row['region_rank'])}"
...
def build_candidate_seed_success_table(
    ...
    seed_nonzero_eps: float = 1e-12,
) -> pd.DataFrame:
    """
    One row per selected rerun candidate point.

    For conjunction:
        correct = validation_seed_conjunction > seed_nonzero_eps
    """
    require_columns(
        point_seed_df,
        [
            ...
            "Percent",
            "Seed",
            "validation_seed_conjunction",
        ],
        "point_seed_lookup",
    )

    work = point_seed_df.copy()
    work["validation_seed_conjunction"] = pd.to_numeric(
        work["validation_seed_conjunction"],
        errors="coerce",
    )
    ...
    for keys, sub in work.groupby(group_cols, dropna=False):
        ...
        valid = sub.dropna(subset=["validation_seed_conjunction"]).copy()
        ...
        correct_mask = valid["validation_seed_conjunction"] > float(seed_nonzero_eps)
        n_correct = int(correct_mask.sum())

        seed_values = valid["validation_seed_conjunction"].astype(float)

        success_rate = n_correct / n_total if n_total > 0 else np.nan

        row = {
            **key_dict,
            "expected_validation_pattern": "nonzero_conjunction",
            "n_correct": n_correct,
            "n_total": n_total,
    ...
    out = out.sort_values(
        ["pair_name", "region_rank", "point_type_order", "Layer", "NoiseStd", "Percent"]
...
def build_region_seed_success_table(candidate_success_df: pd.DataFrame) -> pd.DataFrame:
    ...
    group_cols = ["pair_name", "pair_a", "pair_b", "region_rank"]
    ...
    out = pd.DataFrame(rows)
    out = out.sort_values(["pair_name", "region_rank"]).reset_index(drop=True)
    return out
...
def build_paper_region_summary_table(
    ...
    region_cols = [
        ...
        "peak_noise",
        "peak_percent",
        "validation_mean_conjunction",
        "validation_median_conjunction",
        "validation_min_conjunction",
        "validation_max_conjunction",
        "mean_seed_nonzero_rate",
        "min_seed_nonzero_rate",
        "max_seed_nonzero_rate",
    ...
    if region_seed_success_df is not None and not region_seed_success_df.empty:
        ...
        out = out.merge(
            ...
            on=["pair_name", "region_rank"],
            how="left",
        )

    if "pair_name" in out.columns and "region_rank" in out.columns:
        out = out.sort_values(["pair_name", "region_rank"]).reset_index(drop=True)
...
def plot_region_validation_mean_conjunction(region_df: pd.DataFrame, outpath: Path, dpi: int = 220):
    col = "validation_mean_conjunction"
    ...
    work = work.sort_values(["pair_name", "region_rank"]).reset_index(drop=True)

    x = np.arange(len(work))
    y = pd.to_numeric(work[col], errors="coerce").to_numpy(dtype=float)

    fig, ax = plt.subplots(figsize=(max(8, len(work) * 0.75), 5))
    ax.bar(x, y)
    add_zero_line(ax, "y")

    ax.set_ylabel("Validation mean conjunction")
    ax.set_title("Validation conjunction magnitude by discovered region")
    ax.set_xticks(x)
    ax.set_xticklabels(work["label"], rotation=45, ha="right")

    for xi, yi in zip(x, y):
        if np.isfinite(yi):
            ax.text(xi, yi + 0.003, f"{yi:.3f}", ha="center", va="bottom", fontsize=8)
...
def plot_region_mean_seed_nonzero_rate(
    region_df: pd.DataFrame,
    outpath: Path,
    seed_nonzero_rate_threshold: float = 0.70,
    dpi: int = 220,
):
    col = "mean_seed_nonzero_rate"
    if col not in region_df.columns:
        print(f"[SKIP] {outpath.name}: missing {col}")
        return
    ...
    ax.bar(x, y)
    ax.set_ylim(0, 1.05)
    add_threshold_line(ax, seed_nonzero_rate_threshold, f"threshold = {seed_nonzero_rate_threshold:.2f}")

    ax.set_ylabel("Mean validation seed nonzero rate")
    ax.set_title("Validation seed support by discovered region")
    ax.set_xticks(x)
    ax.set_xticklabels(work["label"], rotation=45, ha="right")
...
def plot_candidate_seed_success_count(
    ...
    ax.set_title("Seed-level validation success by selected condition")
    ax.set_xticks(x)
...
def plot_candidate_success_rate(
    candidate_df: pd.DataFrame,
    outpath: Path,
    dpi: int = 220,
):
    ...
    ax.set_title("Seed-level validation success rate by selected condition")
    ax.set_xticks(x)
    ax.set_xticklabels(work["label"], rotation=75, ha="right", fontsize=8)
...
def plot_discovery_vs_validation_conjunction(
    point_df: pd.DataFrame,
    outpath: Path,
    dpi: int = 220,
):
    required = ["discovery_mean_conjunction", "validation_mean_conjunction"]
    if any(c not in point_df.columns for c in required):
        print(f"[SKIP] {outpath.name}: missing discovery/validation conjunction columns")
        return
    ...
    for point_type, sub in work.groupby("point_type"):
        ax.scatter(
            sub["discovery_mean_conjunction"].to_numpy(dtype=float),
            sub["validation_mean_conjunction"].to_numpy(dtype=float),
            label=point_type,
    ...
    all_vals = np.concatenate([
        work["discovery_mean_conjunction"].to_numpy(dtype=float),
        work["validation_mean_conjunction"].to_numpy(dtype=float),
    ])
    ...
    ax.set_xlim(lo, hi)
    ax.set_ylim(lo, hi)
    ax.set_xlabel("Discovery mean conjunction")
    ax.set_ylabel("Validation mean conjunction")
...
def plot_validation_conjunction_by_region(
    point_df: pd.DataFrame,
    outpath: Path,
    dpi: int = 220,
):
    ...
    for keys, sub in work.groupby(["pair_name", "region_rank"], dropna=False):
        pair_name, region_rank = keys
        grouped.append(sub[col].to_numpy(dtype=float))
        labels.append(f"{pretty_pair_label(pair_name)}\nR{int(region_rank)}")
    ...
    ax.set_ylabel("Validation mean conjunction")
    ax.set_title("Distribution of selected-point validation conjunction by region")
...
def main():
    ...
    ap.add_argument(
        ...
        type=float,
        default=1e-12,
        help="A validation seed is counted correct if conjunction > this value.",
    )
    ap.add_argument(
        "--seed_nonzero_rate_threshold",
        type=float,
        default=0.70,
        help="Threshold line for mean seed nonzero-rate plot.",
    )
    ...
    candidate_success_df = build_candidate_seed_success_table(
        ...
        seed_nonzero_eps=args.seed_nonzero_eps,
    )
    ...
    # Plots
    # -------------------------
    plot_region_validation_mean_conjunction(
        region_df,
        outdir / "paper_region_validation_mean_conjunction.png",
        dpi=args.dpi,
    )

    plot_region_mean_seed_nonzero_rate(
        region_df,
        outdir / "paper_region_mean_seed_nonzero_rate.png",
        seed_nonzero_rate_threshold=args.seed_nonzero_rate_threshold,
        dpi=args.dpi,
    )
    ...
    plot_discovery_vs_validation_conjunction(
        point_df,
        outdir / "paper_discovery_vs_validation_conjunction.png",
        dpi=args.dpi,
    )

    plot_validation_conjunction_by_region(
        point_df,
        outdir / "paper_validation_conjunction_by_region.png",
        dpi=args.dpi,
    )

    print(f"All paper-ready conjunction validation outputs saved under: {outdir}")

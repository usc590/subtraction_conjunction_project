#!/usr/bin/env python3
"""
plot_conjunction_3d_regions_v4.py

Plot true 3D conjunction regions using:
- region_voxels.csv
- region_summary.csv
...
New in v4
---------
1. Keeps the combined "all selected regions together" plot.
2. Also makes separate plots for each selected region.
3. Keeps boundary-only mode.
4. Uses improved plotting defaults:
   - voxel_size = 10
   - voxel_alpha = 0.45
...
Typical usage
-------------
python plot_conjunction_3d_regions_v4.py \
  --region_voxels_csv ./tfce_results/conj3d_targetpairs_region_voxels.csv \
  --region_csv ./tfce_results/conj3d_targetpairs_region_summary.csv \
  --rerun_csv ./tfce_results/conj3d_targetpairs_rerun_candidates.csv \
  --outdir ./tfce_results/conj3d_targetpairs_3dplots_v4 \
  --pairs Phonemic_and_Neologism Semantic_and_Neologism \
  --top_regions 0 \
  --boundary_only \
  --also_full_grid
"""
...
def pretty_pair_title(pair_name: str) -> str:
    if "_and_" in pair_name:
        a, b = pair_name.split("_and_", 1)
        return f"{a} ∧ {b}"
    return pair_name
...
def get_region_size_from_summary(region_summary_pair: pd.DataFrame, region_rank: int):
    if region_summary_pair is None or region_summary_pair.empty:
        return None

    row = region_summary_pair[region_summary_pair["region_rank"] == region_rank]
    if row.empty:
        return None
...
def make_region_legend(region_ranks, region_summary_pair, colors):
    handles = []

    for rr, color in zip(region_ranks, colors):
        size = get_region_size_from_summary(region_summary_pair, rr)
...
def normalize_region_voxels_columns(region_voxels_df: pd.DataFrame):
    ...
    num_cols = [
        "region_rank", "Layer", "NoiseStd", "Percent",
        "idx_layer", "idx_noise", "idx_percent"
    ]
    for c in num_cols:
    ...
    region_voxels_df = region_voxels_df.dropna(
        subset=[
            "pair_name", "region_rank", "Layer", "NoiseStd", "Percent",
            "idx_layer", "idx_noise", "idx_percent"
        ]
    ).copy()
...
def normalize_region_summary_columns(region_df: pd.DataFrame):
    required = ["pair_name", "region_rank"]
    ...
    region_df = region_df.dropna(subset=["pair_name", "region_rank"]).copy()
...
def normalize_rerun_columns(rerun_df: pd.DataFrame):
    ...
    rerun_df = rerun_df.dropna(
        subset=["pair_name", "region_rank", "point_type", "Layer", "NoiseStd", "Percent"]
    ).copy()
...
def extract_boundary_voxels(region_df: pd.DataFrame) -> pd.DataFrame:
    ...
    coord_set = set(tuple(x) for x in region_df[idx_cols].drop_duplicates().to_numpy())

    keep_mask = []
    for _, row in region_df.iterrows():
        pt = (int(row["idx_layer"]), int(row["idx_noise"]), int(row["idx_percent"]))
...
def set_full_grid_axes(
    ...
    noise_ticks = [round(x, 1) for x in np.arange(full_noise_min, full_noise_max + 0.001, 0.1)]
    percent_ticks = [round(x, 1) for x in np.arange(full_percent_min, full_percent_max + 0.001, 0.1)]
...
def prepare_region_points_for_plot(region_voxels_pair: pd.DataFrame, selected_regions, boundary_only: bool):
    """
    Returns a dict:
    ...
    for region_rank in selected_regions:
        reg_vox = region_voxels_pair[region_voxels_pair["region_rank"] == region_rank].copy()
...
def plot_pair_3d(
    region_voxels_pair: pd.DataFrame,
    region_summary_pair: pd.DataFrame,
    rerun_pair: pd.DataFrame,
    pair_name: str,
    selected_regions,
    outpath: Path,
    dpi: int = 200,
    ...
    if region_voxels_pair.empty:
        print(f"[SKIP] No region voxels for {pair_name}")
        return

    if not selected_regions:
        print(f"[SKIP] No selected regions for {pair_name}")
        return

    colors = region_color_list(len(selected_regions))

    print(f"[PLOT] {pair_name}")
    plot_points_by_region = prepare_region_points_for_plot(
        region_voxels_pair=region_voxels_pair,
    ...
    if rerun_pair is not None and not rerun_pair.empty:
        rerun_selected = rerun_pair[
            rerun_pair["region_rank"].isin(selected_regions)
        ].copy()
    ...
    title = pretty_pair_title(pair_name)
    ...
    ax.set_title(
        f"{title} | {region_text} | {mode_suffix} | {title_suffix}",
        pad=18,
        fontsize=15,
    )
    ...
    region_handles = make_region_legend(
        ...
        region_summary_pair=region_summary_pair,
        colors=colors,
    )
...
def main():
    ...
    ap.add_argument(
        "--region_voxels_csv",
        required=True,
        help="Path to conjunction *_region_voxels.csv",
    )
    ap.add_argument(
        "--region_csv",
        required=True,
        help="Path to conjunction *_region_summary.csv",
    )
    ap.add_argument(
        "--rerun_csv",
        required=True,
        help="Path to conjunction *_rerun_candidates.csv",
    )
    ap.add_argument(
        "--outdir",
        required=True,
    ...
    ap.add_argument(
        ...
        nargs="+",
        default=None,
        help="Optional pair_name values to plot",
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=3,
        help="Top N regions per pair. Use 0 or negative to plot all available regions.",
    )
    ...
    # Updated defaults requested by you
    ap.add_argument(
        "--voxel_alpha",
        type=float,
        default=0.45,
    ...
    for pair_name in pair_names:
        ...
        region_voxels_pair = region_voxels_df[
            region_voxels_df["pair_name"] == pair_name
        ].copy()

        region_summary_pair = region_df[
        ...
        if not region_summary_pair.empty:
            rank_order = (
                region_summary_pair
            ...
            rank_order = sorted(region_voxels_pair["region_rank"].dropna().unique().tolist())
        ...
        # --------------------------------------------------
        # Combined plot (all selected regions together)
        # --------------------------------------------------
        combined_zoom_name = sanitize_filename(f"{pair_name}_regions3d_v4{suffix}.png")
        combined_zoom_path = outdir / combined_zoom_name

        plot_pair_3d(
            region_voxels_pair=region_voxels_pair,
            region_summary_pair=region_summary_pair,
            rerun_pair=rerun_pair,
            pair_name=pair_name,
            selected_regions=selected_regions,
        ...
        if args.also_full_grid:
            combined_full_name = sanitize_filename(f"{pair_name}_regions3d_v4{suffix}_fullgrid.png")
        ...
        if not args.no_individual_regions:
            for rr in selected_regions:
                ...
                one_zoom_name = sanitize_filename(f"{pair_name}_region{rr}_3d_v4{suffix}.png")
                ...
                if args.also_full_grid:
                    one_full_name = sanitize_filename(f"{pair_name}_region{rr}_3d_v4{suffix}_fullgrid.png")
